...
int checked_element_count(std::size_t size, const char *operation) {
    if (size > static_cast<std::size_t>(INT_MAX)) {
        coooda_core::fail(std::string(operation) + " input is too large for the CUDA elementwise kernel");
    }
...
int grid_stride_grid_size(int n) {
    const int one_thread_blocks = one_thread_per_element_grid_size(n);
...
__global__ void add_scalar_grid_stride_kernel(const float *input, float scalar, float *out, int n) {
    const int stride = blockDim.x * gridDim.x;
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < n; idx += stride) {
        out[idx] = input[idx] + scalar;
    }
...
__global__ void
add_bias_grid_stride_kernel(const float *input, const float *bias, float *out, int n, int bias_size, int op) {
    const int stride = blockDim.x * gridDim.x;
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; idx < n; idx += stride) {
        out[idx] = apply_bias_op(input[idx] + bias[idx % bias_size], op);
    }
...
std::vector<float> run_unary_grid_stride(const std::vector<float> &input, int op, const char *operation) {
    if (input.empty()) {
        return {};
    }
...
std::vector<float> run_binary_grid_stride(
    const std::vector<float> &a,
    const std::vector<float> &b,
    int op,
    const char *operation
...
std::vector<float> run_bias_grid_stride(
    const std::vector<float> &input,
    const std::vector<float> &bias,
    int op,
    const char *operation
    ...
    add_bias_grid_stride_kernel<<<grid_stride_grid_size(n), kBlockSize>>>(
        device_input.data(),
        device_bias.data(),
        device_out.data(),
        n,
...
std::vector<float> unary_relu_grid_stride(const std::vector<float> &input) {
    return run_unary_grid_stride(input, kUnaryRelu, "unary_relu_grid_stride");
}

std::vector<float> unary_gelu_grid_stride(const std::vector<float> &input) {
    return run_unary_grid_stride(input, kUnaryGelu, "unary_gelu_grid_stride");
}

std::vector<float> binary_add_grid_stride(const std::vector<float> &a, const std::vector<float> &b) {
    return run_binary_grid_stride(a, b, kBinaryAdd, "binary_add_grid_stride");
}

std::vector<float> binary_multiply_grid_stride(const std::vector<float> &a, const std::vector<float> &b) {
    return run_binary_grid_stride(a, b, kBinaryMultiply, "binary_multiply_grid_stride");
}
...
std::vector<float> add_scalar_grid_stride(const std::vector<float> &input, float scalar) {
    ...
    const int n = checked_element_count(input.size(), "add_scalar_grid_stride");
    coooda_cuda::memory::DeviceBuffer device_input = coooda_cuda::memory::DeviceBuffer::from_host(input);
    coooda_cuda::memory::DeviceBuffer device_out(input.size());

    add_scalar_grid_stride_kernel<<<grid_stride_grid_size(n), kBlockSize>>>(
        device_input.data(),
        scalar,
        device_out.data(),
        n
    );
    COODA_CUDA_CHECK_LAST("add_scalar_grid_stride");
    return device_out.copy_to_host();
}

std::vector<float> add_bias_grid_stride(const std::vector<float> &input, const std::vector<float> &bias) {
    return run_bias_grid_stride(input, bias, kBiasPlain, "add_bias_grid_stride");
}

std::vector<float> add_bias_relu_grid_stride(const std::vector<float> &input, const std::vector<float> &bias) {
    return run_bias_grid_stride(input, bias, kBiasRelu, "add_bias_relu_grid_stride");
}

std::vector<float> add_bias_gelu_grid_stride(const std::vector<float> &input, const std::vector<float> &bias) {
    return run_bias_grid_stride(input, bias, kBiasGelu, "add_bias_gelu_grid_stride");
}
