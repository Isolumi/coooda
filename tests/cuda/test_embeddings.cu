...
int main() {
    return coooda_core::test::run_tests({
        {"embeddings_cuda_lookup_known_values", []() {
             ...
             require_vector_close(
                 ...
                 "embedding lookup grid stride known values"
             );
         }},

        {"embeddings_cuda_token_position_known_values", []() {
             require_vector_close(
             ...
             require_vector_close(
                 ...
                 "token position embedding grid stride known values"
             );
         }},

        {"embeddings_cuda_repeated_and_boundary_ids", []() {
             require_vector_close(
        ...
        {"embeddings_cuda_empty_tokens", []() {
             coooda_core::test::require(
                 coooda_cuda::nn::embedding_lookup_baseline({}, {1.0f, 2.0f, 3.0f, 4.0f}, 2).empty(),
                 "empty lookup"
             );
             coooda_core::test::require(
                 coooda_cuda::nn::token_position_embedding_baseline({}, {1.0f, 2.0f}, {}, 2).empty(),
                 "empty token position embedding"
             );
         }},

        {"embeddings_cuda_seeded_output_is_finite", []() {
             ...
             const std::vector<float> out =
                 coooda_cuda::nn::token_position_embedding_baseline({7, 3, 0, 6}, token_table, position_table, 6);
             const std::vector<float> grid_out =
                 coooda_cuda::nn::token_position_embedding_grid_stride({7, 3, 0, 6}, token_table, position_table, 6);

             coooda_core::test::require(out.size() == 24, "seeded embedding output size");
             require_vector_close(out, grid_out, "seeded token position grid stride");
             for (const float value : out) {
                 coooda_core::test::require(std::isfinite(value), "embedding output should stay finite");
             }
         }},

        {"embeddings_cuda_rejects_invalid_inputs", []() {
             require_core_error(
                 []() {
                     (void)coooda_cuda::nn::embedding_lookup_baseline({0}, {1.0f, 2.0f, 3.0f}, 2);
                 },
                 "lookup should reject table not divisible by dim"
             );
             require_core_error(
                 []() {
                     (void)coooda_cuda::nn::embedding_lookup_baseline({2}, {1.0f, 2.0f, 3.0f, 4.0f}, 2);
                 },
             ...
             require_core_error(
                 []() {
                     (void)coooda_cuda::nn::embedding_lookup_baseline({}, {}, 0);
                 },
